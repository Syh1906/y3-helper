...
export class GameSessionManager extends vscode.Disposable {
    ...
    // 每个 Client 独立的日志记录（支持多游戏窗口按 slot 选择日志）
     ...
     * - 写入 client 级日志（多窗口按 slot 选择日志的基础）
    ...
    async stopGame(params: any = {}): Promise<any> {
        ...
        if (hasClientSlot) {
            ...
            const boundSession = this.currentSession?.client === client
                ? this.currentSession
                : undefined;
            await this.forceQuitClient(client);

            // 指定的是当前会话窗口时，保持默认停止路径的日志和 MCP 状态清理语义。
            if (boundSession && this.currentSession === boundSession) {
                await this.finishCurrentSession(boundSession);
            }
        ...
        await this.finishCurrentSession(session);
    ...
    /**
     * 清理当前游戏会话，并同步本地 Y3Runtime MCP 连接状态。
     */
    private async finishCurrentSession(session: GameSession): Promise<void> {
        session.status = 'stopped';
        session.logManager.cleanup();
        this.currentSession = undefined;

        // 游戏停止后主动刷新连接，让下次启动时能够检测并重连 y3runtime。
        try {
            await getMcpHub()?.restartConnection('y3runtime');
        } catch {
            // 保持原有停止语义：MCP 状态刷新失败不阻断游戏会话清理。
        }
    }
    ...
    dispose(): void {
        ...
        // 扩展卸载时恢复客户端输出并关闭所有按窗口创建的日志流。
        for (const [client, logManager] of this.clientLogManagers) {
            const originalPrint = this.originalClientPrints.get(client);
            if (originalPrint) {
                client.print = originalPrint;
            }
            try {
                logManager.cleanup();
            } catch {
                // 单个日志流清理失败不应阻断其他客户端资源回收。
            }
        }
        this.clientLogManagers.clear();

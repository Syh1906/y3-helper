...
本文件记录 `y3-helper-xiaowei` 小为版的重要变更。
...
- 侧边栏 `MCP Server` 节点新增当前运行状态说明，并增加 `Agent 接入中心` 入口，用于区分“本地 MCP Server 启停”和“Codex / Claude 项目 MCP 配置启用状态”。
- Codex / Claude 项目 MCP 配置更新时保留用户自定义字段，只更新 Y3-Helper 管理的三项 MCP 必要字段。

### 修复

- 修复启动或停止 MCP Server 后侧边栏状态不刷新的问题。
- `AGENTS.md` / `CLAUDE.md` 改为托管块更新：Y3-Helper 只替换托管块内内容，保留块外用户扩展；旧版完整生成文件后追加的用户内容会自动迁移。

## [1.0.8] - 2026-07-09

### 修复

- 修复 Agent 接入中心在 MCP 启动或停止后继续显示旧状态的问题；执行启停后会重新打开接入中心并刷新状态。
- MCP 启动、停止和已运行提示会说明外部 Agent 客户端通常需要重启或刷新后才会更新 MCP 连接状态。

## [1.0.7] - 2026-07-09

### 变更

- AI 开发环境初始化不再生成 `.y3maker/mcp_settings.json`；地图脚本目录改为通过符号链接复用工程根 Codex、Claude、MCP 和 skill 配置。

## [1.0.6] - 2026-07-08

### 变更

- 将插件对外名称调整为“Y3开发助手（小为版）”。
- 将扩展身份调整为 `syh1906.y3-helper-xiaowei`，安装包名称调整为 `y3-helper-xiaowei-1.0.6.vsix`。
- 更新 README、发布说明和构建校验中的品牌描述，移除“本地分叉版”这类面向维护者的表达。

## [1.0.5] - 2026-07-08

### 变更

- 补强 AI 初始化生成的 `AGENTS.md` 导航规则，增加普通玩法、物编、运行验收和 AI/MCP 配置的任务入口。
- 脚本目录 `AGENTS.md` 增加 `main.lua`、`可重载的代码.lua`、`y3/`、`y3-helper/meta/`、`.vscode/`、`.y3maker/`、日志目录的职责边界。

### 修复

- 继续防止 AI 初始化产物写入本机绝对路径，并将真实地图副本验收覆盖到新增导航规则。
...
- 将扩展身份调整为 `syh1906.y3-helper-xiaowei`，作为小为版的版本基线。

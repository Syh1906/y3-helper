...
`build.yml` 在推送 `main` 分支时运行。它会安装依赖、执行 TypeScript 编译检查，并生成 `dist/vsix/y3-helper-xiaowei-<version>.vsix`。工作流上传 artifact 时使用 `y3-helper-xiaowei-${{ github.sha }}.vsix` 作为 artifact 名称。
